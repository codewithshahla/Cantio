...
/**
 * F1: Smart Auto Queue — fetch related tracks for a given videoId.
 * Uses Innertube "Up Next" / related tracks. Deduplicates and filters
 * to return only music-appropriate results (1–10 min, no non-music).
 */
export async function getRelatedTracks(videoId: string, limit: number = 20): Promise<VideoResult[]> {
  const yt = await getYouTube();

  try {
    // Try using getBasicInfo which includes related/watch_next content
    const info = await yt.getBasicInfo(videoId);
    const related = (info as any).related_videos
      ?? (info as any).watch_next_feed
      ?? [];

    const MIN_DURATION = 60;
    const MAX_DURATION = 600;
    const seen = new Set<string>([videoId]); // exclude the source track

    const blacklist = [
      'interview', 'podcast', 'reaction', 'review', 'trailer',
      'news', 'documentary', 'vlog', 'comedy', 'gaming', 'tv',
    ];
    ...
    for (const item of related) {
      ...
      // Duration filter
      if (duration > 0 && (duration < MIN_DURATION || duration > MAX_DURATION)) continue;

      // Blacklist filter
      const titleLower = title.toLowerCase();
      const artistLower = artist.toLowerCase();
      if (blacklist.some(k => titleLower.includes(k) || artistLower.includes(k))) continue;
      ...
      tracks.push({
        ...
        artist,
        duration,
        thumbnail: getThumbnailUrl(item.thumbnail?.contents || item.thumbnail || item.thumbnails)
          || `https://i.ytimg.com/vi/${id}/hqdefault.jpg`,
      });
    }

    // If Innertube related didn't give enough, try YT Music radio/up_next
    if (tracks.length < 5) {
      try {
        const ytm = await getYouTubeMusic();
        const upNext = await (ytm as any).music.getUpNext(videoId);
        const items = upNext?.contents ?? upNext?.items ?? [];
        ...
        console.warn('[youtube.ts] Music up_next fallback failed:', e);
      }
    }

    console.log(`[youtube.ts] Found ${tracks.length} related tracks for ${videoId}`);
    return tracks;
  } catch (error) {
    console.error('[youtube.ts] getRelatedTracks failed:', error);
    return [];
  }
}
